...
function inferPaletteFromColors(colors?: string): Array<{ label: string; hex: string }> {
  ...
  if (lower.includes('dark') || lower.includes('sombre') || lower.includes('noir') || lower.includes('luxe') || lower.includes('gold') || lower.includes('or')) {
  ...
  if (lower.includes('sage') || lower.includes('nature') || lower.includes('zen') || lower.includes('naturel') || lower.includes('bio') || lower.includes('végétal')) {
    return PALETTE_SEEDS.sage;
  }

  if (lower.includes('navy') || lower.includes('marine') || lower.includes('corporate') || lower.includes('professionnel') || lower.includes('sérieux')) {
    return PALETTE_SEEDS.navy;
  }

  if (lower.includes('terracotta') || lower.includes('terre') || lower.includes('rouille') || lower.includes('rust') || lower.includes('artisan')) {
    return PALETTE_SEEDS.terracotta;
  }

  if (lower.includes('neon') || lower.includes('néon') || lower.includes('fluo') || lower.includes('gaming') || lower.includes('tech') || lower.includes('startup')) {
...
function buildCreativeBrief(
  ...
  const typoMap: Record<string, string> = {
    modern: 'Typographie sans-serif contemporaine et clean (Inter, DM Sans) avec whitespace généreux et hiérarchie claire. Approche Vercel/Linear.',
    elegant: 'Association serif + sans-serif raffinée (Playfair Display + Inter). Feel sophistiqué, intemporel et luxueux. Approche hôtellerie de luxe.',
    ...
    minimalist: 'Hiérarchie typo ultra-clean (Helvetica Neue, SF Pro). Retenue, purposeful, maximum de whitespace. Approche Apple.',
    classic: 'Typographie traditionnelle et digne de confiance (Lora, Merriweather). Heritage feel avec grille structurée et élégante.',
    playful: 'Typographie ronde et amicale (Nunito, Quicksand). Ambiance décontractée, accessible et chaleureuse. Couleurs vives.',
    luxury: 'Typographie haute couture (Cormorant Garamond, Didot). Accents dorés, lignes fines, photographie riche. Premium absolu.',
    geometric: 'Typographie géométrique précise (Space Grotesk, Outfit). Formes angulaires, grille rigoureuse. Approche architecturale.',
    editorial: 'Layout magazine avec grande typographie (Fraunces, Source Serif Pro). Content-first, storytelling visuel, asymétrie maîtrisée.',
    futuristic: 'Typographie futuriste et tech (Clash Display, Syne). Accents néon, fond sombre, effets glass. Approche startup tech/SaaS.',
  };

  const nicheHints: Record<string, string> = {
    plumber: 'Show trust badges, emergency availability (24/7), service areas, before/after gallery, customer testimonials with star ratings, and a prominent "Call Now" CTA. Include certifications (RGE, Qualibat) and insurance badges.',
    plombier: 'Badges de confiance (RGE, Qualibat, décennale), disponibilité urgence 24h/7j avec numéro cliquable (format 06 XX XX XX XX), zone d\'intervention sur carte, galerie avant/après en slider, avis clients avec étoiles dorées et photos, devis gratuit en ligne, tarifs transparents, mentions "Artisan certifié". CTA rouge pulsant "Appeler maintenant".',
    restaurant: 'Photo culinaire hero en plein écran avec parallax, menu du jour en typographie manuscrite, carte avec catégories filtrables et photos HD, réservation inline avec sélecteur de date/heure, galerie ambiance en masonry, portrait du chef + histoire, avis TripAdvisor/Google intégrés, horaires par jour, Google Maps embed, liens réseaux sociaux.',
    dentist: 'Highlight services with icons, team photos with credentials, patient testimonials, insurance accepted, online booking CTA (Doctolib-style), before/after gallery, virtual tour, FAQ by treatment type.',
    dentiste: 'Design rassurant (blanc + bleu ciel), services par catégorie (esthétique, implants, orthodontie, pédodontie) avec icônes, équipe avec diplômes et spécialités, prise de RDV Doctolib intégrée, galerie avant/après soins esthétiques, tarifs avec base Sécu, FAQ par soin, visite virtuelle du cabinet, mentions "Conventionné secteur 1/2".',
    photographer: 'Full-width portfolio gallery in masonry grid, category filters (wedding, portrait, corporate), packages/pricing comparison, about with personal portrait, client testimonials, contact form contextual by shooting type.',
    photographe: 'Portfolio masonry plein écran sans marges, catégories (mariage, portrait, corporate, événement) avec filtres, lightbox avec gestes, packages avec comparateur de formules, section "Mon approche" narrative avec portrait, témoignages clients avec galerie associée, formulaire contextuel par type de shooting, blog.',
    lawyer: 'Professional authority: practice areas with icons, attorney profiles with bar admissions, case results/stats, client testimonials, free consultation CTA, legal publications.',
    avocat: 'Design sobre et autoritaire (navy + or/bordeaux), domaines de compétence détaillés avec icônes (droit des affaires, pénal, famille, immobilier), profils avocats avec barreau et spécialités, résultats chiffrés, honoraires et convention d\'honoraires expliqués, consultation gratuite CTA, articles juridiques vulgarisés, mentions déontologiques.',
    'real estate': 'Featured listings with card grid and carousel photos, advanced property search (price, area, DPE), agent profiles, neighborhood guides with stats, mortgage calculator, contact form.',
    immobilier: 'Recherche de biens avec filtres avancés (prix, surface, quartier, DPE énergie), cartes d\'annonces avec carrousel photos, estimation en ligne, alerte email, visite virtuelle 360°, simulateur de prêt, guides acheteur/vendeur/locataire, équipe avec zones géographiques, diagnostics obligatoires mentionnés.',
    couvreur: 'Photos drone de toitures rénovées en hero, services détaillés (couverture, zinguerie, charpente, isolation, étanchéité), badges RGE + Qualibat + décennale, avant/après slider, devis en ligne avec upload photo, zone d\'intervention carte, aides financières expliquées (MaPrimeRénov\', CEE, éco-PTZ), témoignages avec photos de chantier.',
    roofer: 'Aerial drone photography hero, services grid with animations, free inspection CTA, financing options, certifications wall, project gallery with filters, 5-star review carousel, emergency service banner.',
    artisan: 'Galerie portfolio en masonry, story de l\'artisan en timeline, savoir-faire détaillé, certifications et labels qualité, devis gratuit CTA, témoignages avec photos de chantier, blog conseils entretien, zone d\'intervention.',
    boulangerie: 'Photos macro de pains et viennoiseries en hero, story du boulanger et de ses farines, carte des produits illustrée par catégorie, horaires par jour, commande spéciale (gâteaux, traiteur), galerie Instagram, plan d\'accès, mentions "Pain au levain naturel", "Farines bio".',
    coiffeur: 'Vidéo loop de coiffures en hero, galerie avant/après, services détaillés (coupe, coloration, balayage, lissage, barbe) avec tarifs, équipe avec spécialités, prise de RDV en ligne, produits vendus au salon, blog tendances capillaires, programme fidélité.',
    garage: 'Design industriel (fond sombre + accents orange/rouge), services en grille (vidange, freins, pneus, contrôle technique, climatisation), tarifs forfaitaires transparents, marques véhicules acceptées, devis en ligne, véhicule de prêt mentionné, avis Google, horaires et accès.',
    coach: 'Design épuré et inspirant, couleurs apaisantes, hero avec portrait + citation inspirante, parcours et certifications (ICF, PNL), programmes (individuel, groupe, entreprise) avec pricing, témoignages transformations, blog articles développement personnel, réservation séance découverte gratuite.',
    architecte: 'Design minimaliste suisse, projets en grid avec hover reveal (photos, plans, surface, budget), services (construction, rénovation, extension, décoration), processus en 5 étapes visuelles, publications presse, formulaire projet détaillé.',
    fitness: 'Design énergique fond noir + accents néon, vidéo hero d\'entraînement, programmes (musculation, cardio, HIIT, yoga) avec détails, coachs certifiés, planning cours interactif, tarifs abonnement comparateur, transformations avant/après, essai gratuit CTA.',
    veterinaire: 'Design doux (verts + beiges), photos d\'animaux, services (consultation, chirurgie, dentisterie, NAC, urgences), équipe avec spécialités, urgences 24h, prise de RDV, conseils santé animale, pharmacie vétérinaire.',
    ecommerce: 'Hero produit vedette animé, catégories en bento grid visuelles, carrousel nouveautés, badges livraison gratuite/retours/paiement sécurisé, section tendances, newsletter avec réduction, CGV et politique retour dans footer.',
    ...
    fleuriste: 'Design romantique et coloré, hero avec bouquet saisonnier, catalogue par occasion (mariage, deuil, anniversaire, Saint-Valentin), livraison même jour, abonnement floral, galerie de créations, commande en ligne, témoignages.',
    traiteur: 'Photos culinaires premium en hero, menus par événement (mariage, entreprise, anniversaire) avec tarifs par personne, galerie de prestations, témoignages clients, devis personnalisé, produits de saison mis en avant.',
    electricien: 'Badges Consuel + Qualifelec + décennale, services (installation, rénovation, dépannage, domotique, borne de recharge), urgence 24h, devis gratuit, avant/après, normes NF C 15-100 mentionnées, aides financières.',
    paysagiste: 'Galerie de réalisations plein écran (jardins, terrasses, piscines), services détaillés avec visuels, processus de création en étapes, devis sur mesure, entretien annuel, inspirations saisonnières, zone d\'intervention.',
  };
  ...
  if (colors) {
    parts.push('', `The client has a brand direction in mind: ${colors}. You take this as inspiration and build a refined palette around it.`);
  }
...
const DESIGN_GODS: Array<{ url: string; style: string; bestFor: string }> = [
  { url: 'https://linear.app', style: 'modern-saas', bestFor: 'Hero avec gradient mesh, layout ultra-clean, animations smooth, dark mode élégant' },
  { url: 'https://stripe.com', style: 'modern-corporate', bestFor: 'Hiérarchie typographique parfaite, grids innovants, couleurs vibrantes sur fond clair, micro-interactions subtiles' },
  { url: 'https://vercel.com', style: 'minimalist-tech', bestFor: 'Whitespace maximal, typographie bold, sections alternées noir/blanc, CTAs nets' },
  { url: 'https://www.apple.com/fr', style: 'minimalist-luxury', bestFor: 'Photos produit immersives, scroll storytelling, typography large et aérée, simplicité absolue' },
  { url: 'https://raycast.com', style: 'modern-product', bestFor: 'Bento grid, glassmorphism, gradients subtils, feature showcase élégant' },
  { url: 'https://framer.com', style: 'creative-modern', bestFor: 'Layout asymétrique, animations reveal, typographie display bold, design playground' },
  { url: 'https://monopo.co.jp', style: 'editorial-agency', bestFor: 'Layout éditorial magazine, typo géante, portfolio immersif, asymétrie maîtrisée' },
  { url: 'https://www.awwwards.com/sites/obys-agency', style: 'avant-garde', bestFor: 'Layout expérimental, animations complexes, identité visuelle forte' },
  { url: 'https://www.sezane.com', style: 'elegant-ecommerce', bestFor: 'E-commerce éditorial, photographie lifestyle, storytelling brand, palette douce' },
  { url: 'https://www.aesop.com/fr', style: 'luxury-minimal', bestFor: 'Minimalisme luxueux, palette restreinte, typographie serif raffinée, contenu immersif' },
];
...
async function findAwardWinningSites(
  niche: string,
  tavilyKey: string,
): Promise<Array<{ url: string; why: string }>> {
  const queries = [
      ...
      try {
        ...
        return data.results
          .filter((r) => {
            ...
            return !u.includes('dribbble.com') &&
  ...
  if (tavilyKey && results.length < 3) {
    try {
      ...
      for (const site of awardSites.slice(0, 2)) {
        if (results.some((r) => r.sourceUrl === site.url)) continue;
...
const CURATED_REFERENCES: Record<string, DesignReference[]> = {
  plumber: [
    { title: 'Plumbing Pro – Hero avec compteur d\'interventions', description: 'Header sticky transparent, hero plein écran avec photo technicien, compteurs animés (2500+ interventions, 98% satisfaction), CTA "Urgence 24/7" en rouge pulsant. Sections: services en bento grid 3x2 avec icônes outline, galerie avant/après en slider, avis Google intégrés avec étoiles dorées, zone d\'intervention sur carte interactive, formulaire devis en 3 étapes.', source: 'Dribbble' },
    { title: 'AquaFix – Design épuré bleu marine', description: 'Navigation minimaliste, hero asymétrique avec vidéo en background, badges de confiance (RGE, Qualibat) en bandeau. Pricing transparent avec 3 forfaits, FAQ accordéon, footer riche avec horaires et numéro cliquable.', source: 'Awwwards' },
    { title: 'FlowMaster Plumbing – Landing page conversion', description: 'Above the fold optimisé : titre accrocheur, sous-titre bénéfice, numéro de téléphone géant, formulaire inline. Social proof immédiat avec logos partenaires. Témoignages vidéo, garantie satisfaction mise en avant.', source: 'Behance' },
  ],
  plombier: [
    { title: 'PlombExpress – Site vitrine artisan français', description: 'Design français authentique avec header bleu/blanc, hero montrant un artisan souriant, badges "Artisan certifié RGE", "Devis gratuit en 2h", "Intervention 7j/7". Tarifs transparents, zone d\'intervention par code postal, avis clients avec photos, mentions légales RGPD complètes.', source: 'Dribbble' },
    { title: 'Maison Plomberie – Élégance artisanale', description: 'Approche premium pour plombier haut de gamme. Fond crème, typographie serif élégante, photos de salles de bain rénovées. Services détaillés : installation, rénovation, dépannage. Portfolio de réalisations avec filtres, blog conseils, chat en direct.', source: 'Pinterest' },
  ],
  restaurant: [
    { title: 'Gusto – Restaurant landing page immersive', description: 'Plein écran photo culinaire avec parallax, navigation overlay transparente, menu du jour en typographie manuscrite animée. Sections : carte avec catégories filtrables et photos, réservation inline avec calendrier, galerie ambiance masonry, portrait du chef avec histoire, avis TripAdvisor intégrés. Footer avec plan Google Maps embed.', source: 'Awwwards' },
    { title: 'La Table d\'Or – Gastronomie française', description: 'Design dark élégant avec accents dorés, hero vidéo du restaurant, typographie Playfair Display + Inter. Menu dégustation présenté comme un parcours visuel, section "Notre philosophie" avec timeline, réservation OpenTable intégrée, galerie Instagram feed en temps réel.', source: 'Dribbble' },
    { title: 'Bistro Modern – Fast casual branding', description: 'Style jeune et dynamique, couleurs vives, illustrations custom, menu interactif avec badges (végétarien, sans gluten), commande en ligne, programme fidélité, stories du personnel.', source: 'Behance' },
  ],
  dentist: [
    { title: 'SmileCare – Clinique dentaire premium', description: 'Fond blanc immaculé, accents bleu ciel rassurant, hero avec sourire naturel. Navigation par type de soin (esthétique, implants, orthodontie). Profils dentistes avec diplômes, visite virtuelle du cabinet, prise de RDV Doctolib intégrée, FAQ par soin, témoignages patients avant/après.', source: 'Dribbble' },
    { title: 'DentaLux – Expérience patient digitale', description: 'Design app-like avec micro-animations, parcours patient interactif, simulateur de résultat blanchiment, formulaire premier RDV intelligent, espace patient sécurisé, blog santé bucco-dentaire.', source: 'Awwwards' },
  ],
  dentiste: [
    { title: 'Cabinet Sourire – Dentiste français moderne', description: 'Design rassurant et professionnel adapté au marché français. Hero avec photo d\'équipe, badges "Conventionné", "Tiers payant", "Urgences". Soins détaillés avec tarifs Sécu/complémentaire, équipe avec spécialités, prise de RDV Doctolib, accès PMR mentionné, mentions RGPD.', source: 'Pinterest' },
  ],
  photographer: [
    { title: 'LensArt – Portfolio photographe minimaliste', description: 'Grid masonry plein écran sans padding, navigation discrète qui disparaît au scroll, lightbox avec gestes swipe. Catégories : mariage, portrait, corporate, événement. À propos avec portrait et philosophy, packages avec comparaison, formulaire de contact contextuel par type de shooting.', source: 'Awwwards' },
    { title: 'Capteur d\'Âmes – Photographe auteur', description: 'Design éditorial magazine-like, grandes photos en pleine page avec textes overlay, scroll horizontal pour les séries, dark mode par défaut, animations reveal au scroll, musique de fond optionnelle.', source: 'Behance' },
  ],
  photographe: [
    { title: 'Studio Lumière – Photographe mariage France', description: 'Style romantique et aérien, tons pastels, typographie manuscrite + sans-serif. Hero carrousel de mariages, section "Mon approche" narrative, galeries triées par lieu (Paris, Provence, Côte d\'Azur), tarifs "à partir de", blog mariages, formulaire avec date et lieu.', source: 'Pinterest' },
  ],
  lawyer: [
    { title: 'LexPro – Cabinet d\'avocats corporate', description: 'Design autoritaire et sobre, fond navy + or, typographie serif imposante. Hero avec skyline, domaines d\'expertise en grid iconique, profils avocats avec barreaux et spécialités, résultats chiffrés (500+ affaires gagnées), publications juridiques, formulaire consultation gratuite.', source: 'Dribbble' },
  ],
  avocat: [
    { title: 'Maître & Associés – Cabinet français', description: 'Design conforme aux règles déontologiques du barreau français. Tons bordeaux et crème, domaines de compétence détaillés (droit des affaires, pénal, famille), honoraires et convention d\'honoraires, accès au droit (aide juridictionnelle), prise de RDV, articles juridiques vulgarisés.', source: 'Pinterest' },
  ],
  immobilier: [
    { title: 'NestHome – Agence immobilière premium', description: 'Recherche de biens avec filtres avancés (prix, surface, quartier, DPE), cartes d\'annonces avec carrousel photos, estimation en ligne, alertes email, visite virtuelle 360°, simulateur de prêt, guides acheteur/vendeur, équipe avec zones géographiques.', source: 'Dribbble' },
    { title: 'Pierre & Prestige – Immobilier de luxe', description: 'Design magazine avec photos architecturales plein écran, biens d\'exception en présentation story, vidéo drone, quartiers détaillés avec stats, service conciergerie, off-market sur demande.', source: 'Awwwards' },
  ],
  couvreur: [
    { title: 'ToitPro – Couvreur artisan certifié', description: 'Hero avec drone shot de toiture rénovée, badge RGE + Qualibat + décennale. Services : couverture, zinguerie, charpente, isolation, étanchéité. Avant/après slider, devis en ligne avec upload photo, zone d\'intervention carte, aides financières (MaPrimeRénov\', CEE) expliquées.', source: 'Pinterest' },
  ],
  roofer: [
    { title: 'RoofCraft – Modern roofing company', description: 'Bold design with aerial photography, services grid with animations, free inspection CTA prominent, financing options, certifications wall, project gallery with filters, 5-star review carousel, emergency service banner.', source: 'Dribbble' },
  ],
  artisan: [
    { title: 'L\'Atelier – Artisan multi-services', description: 'Design chaleureux avec textures bois/pierre, portfolio de réalisations en masonry, savoir-faire en timeline chronologique, certifications et labels, devis gratuit, témoignages avec photos de chantier, blog conseils.', source: 'Pinterest' },
  ],
  ecommerce: [
    { title: 'ShopLux – E-commerce premium', description: 'Hero avec produit vedette animé, catégories en bento grid visuelles, carrousel nouveautés, badges livraison gratuite/retours/paiement sécurisé, section tendances, newsletter avec réduction, footer méga-menu avec FAQ, CGV, politique de retour.', source: 'Awwwards' },
  ],
  boulangerie: [
    { title: 'La Mie Dorée – Boulangerie artisanale', description: 'Design chaleureux tons bruns et crème, photos macro de pains et viennoiseries, story du boulanger, nos farines et ingrédients, carte des produits illustrée, horaires par jour, commande spéciale (gâteaux, traiteur), galerie Instagram, plan d\'accès.', source: 'Pinterest' },
  ],
  coiffeur: [
    { title: 'Studio Ciseaux – Salon de coiffure tendance', description: 'Design glamour avec vidéo loop de coiffures, galerie avant/après, services détaillés (coupe, coloration, balayage, lissage), tarifs par prestation, équipe avec spécialités, prise de RDV en ligne, produits vendus au salon, blog tendances capillaires.', source: 'Dribbble' },
  ],
  garage: [
    { title: 'AutoPro Garage – Mécanique automobile', description: 'Design industriel moderne, fond sombre avec accents orange, services en grille (vidange, freins, pneus, contrôle technique), tarifs forfaitaires transparents, marques acceptées, devis en ligne, véhicule de prêt, avis Google, accès et horaires.', source: 'Pinterest' },
  ],
  coach: [
    { title: 'MindShift – Coach de vie / développement personnel', description: 'Design épuré et inspirant, couleurs apaisantes (sage green, lavande), hero avec portrait professionnel et citation, parcours et certifications (ICF), programmes (individuel, groupe, entreprise), témoignages transformations, blog articles, réservation séance découverte gratuite.', source: 'Dribbble' },
  ],
  architecte: [
    { title: 'Studio Archi – Cabinet d\'architecture', description: 'Design minimaliste ultra-clean, fond blanc pur, typographie suisse, projets en grid avec hover reveal des détails. Chaque projet : photos, plans, description, surface, budget. Services (construction, rénovation, extension, décoration intérieure), processus en 5 étapes, publications presse, formulaire projet.', source: 'Awwwards' },
  ],
  fitness: [
    { title: 'IronPulse – Salle de sport / coaching', description: 'Design énergique et bold, fond noir avec accents néon, vidéo hero d\'entraînement, programmes (musculation, cardio, HIIT, yoga), coachs avec certifications, planning des cours interactif, tarifs abonnement avec comparateur, transformation clients avant/après, essai gratuit CTA.', source: 'Behance' },
  ],
  veterinaire: [
    { title: 'VétoCare – Clinique vétérinaire', description: 'Design doux et rassurant, tons verts et beiges, photos d\'animaux mignons, services (consultation, chirurgie, dentisterie, NAC), équipe véto avec spécialités, urgences 24h, prise de RDV en ligne, conseils santé animale, pharmacie, assurance animaux.', source: 'Pinterest' },
  ],
};

async function fetchDesignReferences(
  niche: string,
  tavilyKey?: string,
...
export function createStitchDesignTool(
  ...
  return tool({
    ...
    parameters: z.object({
      ...
      typography_style: z
        .enum(['modern', 'elegant', 'bold', 'minimalist', 'classic', 'playful', 'luxury', 'geometric', 'editorial', 'futuristic'])
        .optional()
      ...
      reference_urls: z
        .array(z.string().url())
        .optional()
        .describe('URLs of websites the user wants to use as visual reference (e.g. ["https://stripe.com", "https://linear.app"]). The tool will screenshot these and use them as design inspiration.'),
    }),
